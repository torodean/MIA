...
On Linux, the sequencer requires the \texttt{xdo} features (libxdo and X11), so it is only built when those libraries are found (see section \ref{section:optional_features}). Windows builds provide the input APIs directly and always get the sequencer.
...
\begin{enumerate}
	...
	\item \inlinecode{LISTEN=1} This will open a background thread which listens for the key matching the first character of the value. With \inlinecode{LISTEN=1}, it listens for the '1' key. The character is converted to the platform-specific key code internally, so the same value works on both Linux and Windows. When the key is detected, the sequence will toggle on and off. When toggled, the sequence will stop or restart.
	\item \inlinecode{TYPE=abcdefg} This command will type the text entered. In this case ``abcdefg'' will be typed.
	\item \inlinecode{TYPEHOLD=ab;3000} This command will type the text entered. In this case ``ab'' will be typed. Each character will be held down for 3 seconds.
	\item \inlinecode{PRESS=SPACE} This command will press the space key.
	\item \inlinecode{PRESSRANDNUM=2;5} This command will press a random key between 2 and 5. Note that this key press can trigger the LISTEN key and therefore a non-numerical listen key may be needed when using this action.
	\item \inlinecode{PRESSHOLD=SPACE;2000} This command will press the space key and hold it for 2 seconds.
	\item \inlinecode{SLEEP=500} This defines a pause in the sequence. The value is in milliseconds; here the sequence waits 500 ms before the next action.
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{|l|l|X|}
		...
		\hline
		\texttt{LISTEN} & char & A single character. The key matching this character toggles the sequence on and off (stops or restarts it). The character is converted to the platform-specific key code internally. \\
		\hline
		\texttt{TYPE} & String & Any string of characters to be typed as keystrokes. \\
		\hline
		\texttt{TYPEHOLD} & String;Integer (ms) & Any string of characters to be typed as keystrokes, a semi-colon delimiter, then a delay to hold each character in milliseconds. \\
		\hline
		\texttt{PRESS} & Enum (SpecialButton) & See section \ref{SpecialButton-Enumeration}\\
		\hline
		\texttt{PRESSRANDNUM} & Integer pair & MIN;MAX integer pair separated by a semi-colon (e.g., 2;5). Both values must be between 0 and 9, and MIN must be less than or equal to MAX; a range with MAX < MIN is rejected. A random digit in that range is pressed. \\
		\hline
		\texttt{PRESSHOLD} & Enum (SpecialButton);Integer (ms) & See section \ref{SpecialButton-Enumeration} for first input, followed by a semi-colon delimiter, then a delay to hold each character in milliseconds.\\
		\hline
		...
		\hline
		\texttt{CLICK} & Enum (ClickType) & One of: \texttt{LEFT\_CLICK}, \texttt{RIGHT\_CLICK}, \texttt{MIDDLE\_CLICK}. Both the underscore form (\texttt{LEFT\_CLICK}) and the concatenated form (\texttt{LEFTCLICK}) are accepted, case-insensitive. \\
		\hline
		\texttt{CLICKHOLD} & Enum (ClickType);Integer (ms) & Same values as \texttt{CLICK}, followed by a semi-colon delimiter, then a delay to hold the click in milliseconds. \\
		\hline
...
The \texttt{SpecialButton} enumeration defines a set of special, non-alphanumeric button types. These typically correspond to keys that cannot be represented as single characters and are used in scenarios where such special key input needs to be identified or handled explicitly. Some of these also refer to mouse input which does not fall nicely into the category of a mouse-press (such as using the scroll wheel).
...
\begin{itemize}
	...
	Represents the Space-bar key, used to insert a space character or activate UI elements.
	
	\item \textbf{NUM\_LOCK} \hfill \\
	Represents the Num Lock key, which toggles the numeric keypad input mode.
	
	\item \textbf{SCROLL\_UP} \hfill \\
	Represents a single upward mouse wheel scroll action.
	
	\item \textbf{SCROLL\_DOWN} \hfill \\
	Represents a single downward mouse wheel scroll action.
\end{itemize}
...
\noindent When used with \inlinecode{PRESS} or \inlinecode{PRESSHOLD}, the button is specified by name. Matching is case-insensitive. The accepted values are listed above. Keys with an underscore also accept the concatenated form without it.


\section{Notes on Using The Sequencer}

The sequencer should still be considered experimental and is under ongoing improvement. It is important to follow the scheme for defining sequences above closely, with all needed parameters defined properly. In particular, no two sequences should share a name; the program does not currently detect or handle duplicate sequence names.
