...
\begin{itemize}\itemsep0em
	...
	\item \textbf{Linux:} All file playback goes through libVLC (from \texttt{libvlc}). Only mp3 is in the supported set so far. The player creates a fresh VLC media player for each playback and drives the volume through libVLC's audio API, which supports fades for every supported file type. libVLC warnings and errors are captured through a logging callback and routed to the application logger with \texttt{[VLC, WARNING|ERROR]} tags (falling back to the default log file when no application context is set), instead of being printed to stderr. The \texttt{libvlc-dev} package is required to build; the setup script installs it. The system beep works through \texttt{/dev/console}.
\end{itemize}
