...
\subsection{Cygwin \& Windows Specifics}

If you are using the Python plotting library on Windows through Cygwin, you will need to have Xming running. Matplotlib uses a graphical backend to display plots, and Cygwin does not provide a native Windows display server for X11 applications. Xming provides the X server that Matplotlib can connect to, allowing plots to be displayed normally. There may be other alternative methods of getting this working, but using Xming is the one that was tested. You will also have to configure Cygwin to use Xming by exporting the correct \texttt{DISPLAY} variable as shown below:
\begin{lstlisting}[style=terminalstyle]
awtorode@KernalSanders /cygdrive/s/projects/git/MIA
$ ./bin/apps/mia_test/testGameUI.py
Traceback (most recent call last):
  File "/cygdrive/s/projects/git/MIA/./bin/apps/mia_test/testGameUI.py", line 84, in <module>
    root = tk.Tk()
