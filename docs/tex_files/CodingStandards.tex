...
\section{Engineering Principals}

Many `coding standards' are based around preference or visual desires for code. These rules are nice to have for consistency, however most people tend to have somewhat differing opinions about what preferences they have. A good coding standard should pertain to things outside this realm of preferential stylistic choices and rather focus on principals that actually improve the robustness of the systems being created. Many of the standards written in the sections following this are of the former kind (stylistic, preference, visual consistency, etc). However, the principals in this section are designed to be rules which would benefit safety critical code. The idea of differentiating rules like this come from a document I found which outlines rules specifically designed for safety critical code \cite{Rules_For_Developing_Safety_Critical_Code}. In this section, I outline rules of this sort which still fit within the scope of the MIA project.


\subsection{Function Size Constraint}

Functions must remain small enough to be reviewed as a single logical unit. As a practical limit, a function should not exceed roughly one printed page when formatted with one statement and one declaration per line (approximately 60 lines of code). \cite{Rules_For_Developing_Safety_Critical_Code}

Each function (or method) should be a single unit in the code. It is the smallest unit of behavior and provides a specific purpose. Part of understanding the purpose of a function (especially the context of why it exists, how it's used, etc) lies within the associated documentation and comments. However, the specific functionality should be able to be understood as a standalone unit by the code itself. When a function is multiple pages (or screens), comprehension and readability degrades and potential issues can be masked or overlooked. Large functions typically indicate excessive responsibility - that the unit is doing too much. Enforcing a strict size limit forces clearer separation of concerns and improves reviewability, testability, and long-term maintainability.



\subsection{Return Value and Parameter Validation}

The return value of every non-void function must be explicitly handled by the caller and parameters should be checked for validity within each function. In cases where responding to an error case would be no different than to a success, casting the function return to void is acceptable - as it indicates that the programmer explicitly chose to ignore the return values.

Parameter validation is important for a few reasons. First, it provides a clear check for potential error cases. Second, it forces the developer to think about the possible input possibilities (both desired and potentially not) to help ensure they understand the consequences of different input parameters. Third, parameter validation enforces defensive boundaries between components. Functions should not rely on external guarantees about input correctness. By validating inputs at the point of entry, the code establishes explicit preconditions, prevents undefined behavior, and localizes faults to their source rather than allowing them to propagate unpredictably through the system. 

By explicitly handling function return values, the code ensures that errors are detected and addressed immediately, rather than silently ignored. This enforces propagation of failure information, allows higher-level logic to respond appropriately, and prevents subtle bugs from spreading through the system. Even when a return value is intentionally ignored, doing so explicitly documents the decision, making it clear that it is a conscious, justified exception rather than an oversight.
...
\begin{itemize}\itemsep0em
    ...
    \item \textbf{Header include order}: When a source or header file includes other files, group the includes in a fixed order, separated by blank lines:
        \begin{enumerate}\itemsep0em
        \item The associated (pairing) header first. In a \texttt{.cpp} file this is the matching \texttt{.hpp}. This makes missing dependencies in the pairing header surface immediately.
        \item System headers (e.g., \texttt{<iostream>}, \texttt{<vector>}).
        \item Third-party library headers (e.g., \texttt{<gtest/gtest.h>}).
        \item Other MIA headers (project headers outside the pairing header).
        \end{enumerate}
...
\subsubsection*{Doxygen tag forms}

Use standard Doxygen tag forms throughout. The authoritative Doxygen comments live in header files; for methods implemented in \texttt{.cpp} files that are not declared in a header, add the Doxygen comment in the \texttt{.cpp}.
\begin{itemize}\itemsep0em
    \item \texttt{@brief} - short summary.
    \item \texttt{@param name description} - one per parameter, in order.
    \item \texttt{@return description} - for non-void, non-constructor functions.
    \item \texttt{@throws type description} - for any function that may throw. Use \texttt{@throws}, not \texttt{@throw}.
    \item \texttt{@note}, \texttt{@see}, \texttt{@code}/\texttt{@endcode} as needed.
\end{itemize}
Do not use the legacy bracketed forms \texttt{@param name[type] - ...} or \texttt{@return [type] - ...}; those are being phased out. At the time of writing this, the legacy form is common throughout the project.

\subsubsection*{File headers}

Every \texttt{.cpp} and \texttt{.hpp} file begins with a Doxygen header block. Required tags: \texttt{@file} (followed by the file's basename), \texttt{@author}, \texttt{@date} (\texttt{MM/DD/YYYY}), and \texttt{@brief} for the one-line summary. Do not use the plain \texttt{Description:} line that appears in many older files; use \texttt{@brief}. A \texttt{@copyright} line is optional and is carried only by the older encryption files.
\begin{lstlisting}[style=cppstyle]
/**
 * @file Timing.hpp
...
\subsubsection*{Block comment shape and \texttt{///} usage}

For multi-line Doxygen and plain block comments, the \texttt{/**} (or \texttt{/*}) and \texttt{*/} delimiters each go on their own line, with body lines prefixed by \texttt{ * }. Never place \texttt{/**} or \texttt{*/} on the same line as body text, and never cram a \texttt{/** ... */} block onto a single physical line. A one-line Doxygen comment is written as \texttt{///}; a multi-line one is the block form.

The choice between \texttt{///} and \texttt{/** ... */} depends on what is being documented and the line count:
\begin{itemize}\itemsep0em
    \item For methods (except trivial getters and setters) and tests, the \texttt{/** ... */} block form is preferred even when the brief is a single sentence, since these comments tend to grow a body or tags later. A single-line \texttt{///} is acceptable, but the block form is the preferred choice.
    \item For trivial methods such as simple getters and setters, \texttt{///} is acceptable and often preferable.
    \item Member variables use \texttt{///} on its own line above the declaration, or \texttt{///<} inline on the same line, for a single-line description. Use the \texttt{/** ... */} block form when the description spans multiple lines.
    \item Other single-line Doxygen (e.g. a one-off declaration that is not a method or test) uses \texttt{///}.
\end{itemize}
For non-Doxygen single-line comments, always use \texttt{//}, never \texttt{/* ... */} for single line comments. Use \texttt{/* ... */} blocks for multi-line non-Doxygen prose.

\subsubsection*{Example Doxygen comments}

For multi-line comments, use the \texttt{/** .. */} comment block. The \texttt{/**} and \texttt{*/} delimiters each go on their own line, and body lines are prefixed with \texttt{ * } (space, asterisk, space).
\begin{lstlisting}[style=cppstyle]
/**
