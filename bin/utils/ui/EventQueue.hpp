...
namespace ui
{
     ...
     * for a single reader.
     *
     * Producers (e.g. a UI listener) push their polled events through
     * addEvents(); the reader reads them through getAllEvents() (non-blocking)
     * or waitForEvents() (blocks until events are available or the queue is
     * closed). Because all producers share one queue, the reader has a single
     * place to wait no matter which producer the events came from.
     *
     * The queue carries its own mutex and condition variable, since producers
     * and the reader run on separate threads. Notifications are sent outside
     * the lock so a woken reader does not immediately block again.
     *
