...
\begin{figure}[htbp]
    ...
    \caption{This shows a very basic tree. The tree begins with the node A0. Once completely satisfied, this unlocks the node A1. When A1 is satisfied, A2, B1, and C1 become available. Further down the tree, A3 requires A2 and B2, while A4 requires A3 and C3. This shows that although there are multiple paths and options to take, certain things require nodes from other paths before they can be unlocked.}
    \label{fig:Basictree}
\end{figure}

Most of the systems will be tied to a tree in some way. This will determine pre-requisites or progress along the game when trying to unlock certain things (new adventurers, new buildings, craftables, mission types, etc). For a basic example of a tree with multiple different systems which interconnect, see Figure \ref{fig:Basictree}.
