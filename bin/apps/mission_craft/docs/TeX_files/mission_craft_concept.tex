...
The player will have the option of where they want to start their adventure. From there, they will slowly explore the map as they do exploration missions (or other related activities). As they explore the map, new missions and milestones will unlock. See chapter \ref{chapter:mapping_concept} for more details.
...
A `tree' will be a simplified term which refers to a structured, permanent progression graph which is made of interconnected nodes - representing unlockable content, roles, mechanics, or systems. Nodes are gated by prerequisites and may depend on multiple prior nodes. Advancement through a tree grants access to new gameplay capabilities and may unlock additional trees. A more technically accurate term for these would be directed acyclic graphs (DAGs), though trees will suffice and better describe the individual parts within the larger system.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{./images/basic_tree.png}
    \caption{This shows a very basic tree with three systems - A, B, and C. The tree begins with the node A0. Once completely satisfied, this unlocks the node A1. When A1 is satisfied, A2, B1, and C1 become available. Further down the tree, A3 requires A2 and B2, while A4 requires A3 and C3. This shows that although there are multiple paths and options to take, certain things require nodes from other paths before they can be unlocked.}
    \label{fig:Basictree}
\end{figure}

Most of the systems will be tied to a tree in some way. This will determine prerequisites or progress along the game when trying to unlock certain things (new adventurers, new buildings, craftables, mission types, etc). For a basic example of a tree with multiple different systems which interconnect, see Figure \ref{fig:Basictree}.
...
At its core, one of MissionCraft's core features will be an extensive crafting system. While the player utilizes buildings and adventurers to collect (acquire, farm, grow, etc) resources, they will also be able to utilize buildings to craft those resources into other items. The crafting system will be designed to both be simplistic (for those looking for a casual game), to comprehensive and expansive (to those looking to dive into the subtleties, depth, etc). 

Crafting systems in games are not new - they appear in many games such as World of Warcraft, Minecraft, KOTOR, No Man's Sky, etc. There are certain common themes among many of the crafting systems that are good, and some which are not. MissionCraft will take inspiration from many other games to create a comprehensive, sensible, and fun crafting system. A few features that the crafting system will satisfy are as follows:

\begin{enumerate}[itemsep=0pt]
	\item The crafting system should be setup in a way where low-level resources do not become obselete or undesired in later game phases. Any item that is obtained early on, should remain useful at higher stages. This will prevent the players from feeling like choices they made early on were not worthwhile.
	\item The crafting system should have a somewhat natural progression for casual players. This could be items which unlock naturally as buildings are unlocked, or as nearby areas are unlocked. At the same time, it should also providing some unique and unnatural possibilities for more challenge-focused players. These could be rarer recipes which are unlocked through more risky play-styles or far-away exploration type things.
	\item The crafting system should contain many items, but those items should all make sense as to how they are obtained (such as wood from trees, sticks from a forest, rocks from a cave, etc).
	\item For items which are staples or needed continuously, methods of automated construction should be available to the players (such as creatable farms to produce food).
\end{enumerate}
